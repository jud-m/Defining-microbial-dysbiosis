...
facet_count <- OTU.filt_rel.df %>%
  filter(Genus != "") %>%
  distinct(Genus) %>%
  nrow()


# Calculate missing facets to make the last row complete
n_cols <- 3  # Number of facet columns
n_missing <- (ceiling(facet_count / n_cols) * n_cols) - facet_count  

# Add dummy Genus entries to balance the last row
OTU.filt_rel.df_padded <- OTU.filt_rel.df %>%
  bind_rows(data.frame(Genus = rep(" ", n_missing), OTU = NA, Abundance = NA, Area_Cond = NA))  # Fill with blanks
...
OTU.filt_rel.df %>% 
 ...
 # facet_wrap(~Genus, ncol = 3, scales = "free_y") +
  facet_wrap_paginate(~Genus, ncol = 3, nrow = 3, page = 1, scales = "free_y")  # Adjust as needed# Free y-axis per facet to avoid blank OTUs

ggsave("figures/Figure4.pdf")
# ggsave("figures/coretaxa2.jpeg")
...
# ggsave("figures/coretaxa1.pdf")
# ggsave("figures/coretaxa1.jpeg")
